...
- **Workbench** owns transcript import, cleanup, enrichment, daemon-built canonical dossier publication, and positive-evidence candidate handoffs. A runbook, ADR, or incident timeline is authored only when its candidate exists; absence creates no N/A artifact or per-session obligation.
...
- [Logbook and Workbench](logbook-and-workbench.md) — artifact Logbook UI/API, canonical dossier snapshots, and candidate-driven optional artifacts.
...
- `docs/adr/0013-canonical-dossier-and-candidate-authoring.md` — the original dossier snapshot and candidate-driven V2 authoring contract.
...
Dogfood Logbook may be **empty after artifact cutover** until Workbench publishes canonical dossiers and supported candidate artifacts — that is expected, not a broken connection.
...
- A session dossier always means the original canonical `SessionDossierDto` presentation. Agents never author its body; the daemon publishes `canonical-session-dossier-v1` snapshots.
- Optional authoring uses one `workbench-authoring-v2` candidate per run, no more than 12 provenance sessions, and typed verbatim claim support.
