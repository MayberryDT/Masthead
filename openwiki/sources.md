...
Live connector state comes from `/settings/hooks`. The daemon exposes the release target set—Codex, Claude Code, Cursor, Grok Build, and OpenCode—as a runtime list, with status, managed config path, endpoint, and whether Settings/Sources can run install, test, or uninstall actions.

The Codex compatibility actions still manage the full release target set, while runtime-specific daemon routes can operate on one non-Codex connector at a time. Live capture can create canonical session identity and runtime-signal rows before transcript import is approved; transcript import remains a separate Sources decision.
