...
export const LLM_NARRATIVE_MAX_OUTPUT_TOKENS = 360;
...
export function buildLlmNarrativeRequest(facts: SessionFacts): LlmNarrativeRequest {
  ...
  return {
    fallback,
    inputText: JSON.stringify(providerPayload(facts, fallback))
  };
}
...
export function parseLlmNarrativeResult(input: {
  ...
  const validation = mergeValidatedNarrative(input.fallback, parsedOutput);
  if (!validation.ok) {
...
function safeText(value: string, maxLength: number): string {
  return value
    ...
    .replace(/\b[A-Z][A-Z0-9_]*(?:API_KEY|TOKEN|SECRET|PASSWORD|ACCESS_KEY|PRIVATE_KEY)\s*=\s*[^\s"'`]+/g, "[redacted-secret]")
