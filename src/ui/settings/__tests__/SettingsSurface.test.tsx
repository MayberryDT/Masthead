...
describe("Settings surface", () => {
  ...
  test("uses shared card entrance motion for settings sections", () => {
    ...
    expect(css).toMatch(/@media \(prefers-reduced-motion: no-preference\) \{[\s\S]*\.observability-console \.session-card,[\s\S]*\.masthead-shell \.session-card,[\s\S]*animation: session-card-created 400ms cubic-bezier\(0\.17, 0\.78, 0\.13, 1\) both;[\s\S]*transform-origin: 50% 100%;/);
