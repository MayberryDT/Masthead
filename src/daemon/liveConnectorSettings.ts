...
export async function runLiveConnectorRoundTrip(
  ...
  for (const runtime of runtimes) {
    ...
    try {
      if (usesManagedHookCommand(runtime)) {
        const command = `MASTHEAD_VERIFY_CONNECTOR=1 ${liveConnectorCommand(config, runtime, endpoint)}`;
        const result = await executeManagedConnectorCommand(
          command,
          JSON.stringify(syntheticPayload(runtime, sourceEventId, testedAt))
        );
        if (!result.ok) {
          failures.push(`${LABELS[runtime]} managed connector command failed${result.detail ? `: ${result.detail}` : ""}`);
        }
        continue;
      }
  ...
  return {
    message: `Connector command verified: Masthead executed managed hook commands or validated managed plugin wiring for ${runtimes.map((runtime) => LABELS[runtime]).join(", ")}. No real user session was generated by this test.`,
    status: "passed",
    testedAt
  };
}
...
function stablePackagedCommandPaths(
  ...
  const currentRoot = join(expandHomeDirectory(options.homeDir ?? homedir()), ".local", "share", "masthead-production", "current");
...
function usesManagedHookCommand(runtime: LiveConnectorRuntime): boolean {
  return runtime === "codex" || runtime === "claude_code" || runtime === "cursor" || runtime === "grok";
}

function executeManagedConnectorCommand(
  command: string,
  stdin: string
  ...
  return new Promise((resolveResult) => {
    ...
    child.on("error", (error) => {
      ...
      resolveResult({ ok: false, detail: error.message });
    });
    child.on("close", (code, signal) => {
      ...
      if (code === 0) resolveResult({ ok: true });
        ...
        ok: false,
        detail: stderr.trim() || (signal ? `terminated by ${signal}` : `exited with code ${code ?? "unknown"}`)
      });
    });
...
export async function installLiveConnector(config: DaemonConfig, runtime: LiveConnectorRuntime): Promise<void> {
  ...
  if (runtime === "codex") {
    await setConnectorActivation(dirname(config.databasePath), "codex", {
      ...
      message:
        "Open Codex and run /hooks to review and trust Masthead hooks, then start or restart a Codex session. Masthead marks this connection Ready after it observes a real live event."
    });
  }
}
