...
type SignalRef = { index: number; ref: string };
...
export function discoverArtifactCandidates(
  ...
  return persistSeeds(db, candidateSeedsForSessions(db, normalizedStrings(sessionIds)));
}
...
export function discoverArtifactCandidatePage(
  ...
  for (const row of rows) {
    ...
    if (hasWorkbenchArtifactCandidateScan(db, { evidenceRevision, sessionId: row.sessionId })) continue;
    seeds.push(...candidateSeedsForSessions(db, [row.sessionId]));
    recordWorkbenchArtifactCandidateScan(db, { evidenceRevision, sessionId: row.sessionId });
    scannedSessionIds.push(row.sessionId);
  }
  const result: {
    ...
    candidates: persistSeeds(db, seeds),
    scannedSessionIds
  };
  ...
  if (runbookReady(signals)) {
    seeds.push({
      ...
      signalEvidenceRefs: normalizedStrings([
        ...
        ...signals.changeRefs.map(refValue),
        ...signals.verificationRefs.map(refValue)
      ]),
...
function extractSessionSignals(db: MastheadDatabase, sessionId: string): SessionSignals {
  ...
  for (const item of iterateSessionTranscriptItems(db, { order: "asc", sessionId })) {
    ...
    const ref = { index, ref: item.itemId };
...
function runbookReady(signals: SessionSignals): boolean {
  return signals.failureRefs.some((failure) =>
    signals.changeRefs.some((change) =>
      change.index > failure.index &&
      signals.verificationRefs.some((verification) => verification.index > change.index)
    )
  );
}
...
function proposalHasKindSignals(
  ...
  if (kind === "runbook") {
    return (
      ...
      countAcrossSessions((session) => session.changeRefs) > 0 &&
      countAcrossSessions((session) => session.verificationRefs) > 0
    );
  }
  if (kind === "adr") {
    return (
...
function isPassedVerification(item: SessionTranscriptItem, normalized: string): boolean {
  if (item.kind === "tool_result" && item.status === "succeeded" && (item.exitCode ?? 0) === 0) return true;
  if (item.kind === "checkpoint" && /(?:verification|test|check).*pass|pass.*(?:verification|test|check)/.test(normalized)) {
    return true;
  }
  return /\b(?:verified|verification passed|tests? passed|check passed)\b/.test(normalized);
}
...
function normalizeProposedSignature(signatureKey: string, signals: SessionSignals[]): string {
  ...
  const available = new Set(signals.flatMap((signal) => signal.signatures));
  if (!/^error:[a-z0-9]+:[a-z0-9]+(?:-[a-z0-9]+)*$/.test(normalized) || !available.has(normalized)) {
