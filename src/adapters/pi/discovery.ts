...
export function piCandidatePaths(context: DiscoveryContext): AdapterPathCandidate[] {
  // Redirected/test homes must stay under context.homeDir and never escape via PI_HOME.
  const envHome = context.homeDir === homedir()
    ? process.env.MASTHEAD_PI_HOME ?? process.env.PI_HOME
    : undefined;
  const roots = [
    envHome,
    join(context.homeDir, ".pi"),
    join(context.homeDir, ".local", "share", "pi"),
    join(context.homeDir, ".config", "pi")
  ].filter(Boolean) as string[];
  return roots.map((relativePath) => ({
    confidence: "heuristic",
    contentKind: "jsonl-tree",
    maxDepth: 5,
    purpose: "Pi local session history",
    relativePath,
    runtime: "pi",
    sourceKind: "jsonl"
  }));
