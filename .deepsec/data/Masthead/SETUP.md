# Agent setup for `Masthead`

This is a deepsec scanning workspace. Project `Masthead` was just registered
(target: `..`). Setup is incomplete — `data/Masthead/INFO.md`
...
2. **Fill in `data/Masthead/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id Masthead
pnpm deepsec process --project-id Masthead
```
