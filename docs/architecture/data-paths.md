...
Only one writable daemon may own a canonical SQLite database path. Owners publish unique token records inside `<database-path>.lock`; stale recovery and release remove only that token's record. A second writable daemon targeting the same database fails through normalized or symlink aliases.

The historical data-directory invariant remains authoritative during and after the legacy-lock transition: exactly one writable daemon may own a canonical data directory, even when two configurations name different SQLite files. Distinct writable databases therefore require distinct data directories. A writable daemon requires its database to be physically inside its canonical data directory and exclusively owns the old-readable `<data-dir>/runtime/database.lock` for its lifetime. This prevents old and new writers from overlapping in the supported/default layout without a guardian process. An arbitrary old binary launched with a cross-directory `MASTHEAD_DB_PATH` override cannot be made to honor a new lock protocol; that configuration is unsupported and the old daemon must be stopped before transition. Read-only bridges do not open SQLite for writing and do not take either writer guard.
