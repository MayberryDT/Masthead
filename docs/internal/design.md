...
This is the master design source for Masthead. The repo intentionally uses the lowercase filename `docs/internal/design.md`; tools or agents that expect Google's uppercase `DESIGN.md` format should be pointed at this file.

Historical product requirements live in `docs/archive/prd.md`. Prefer README, OpenWiki, CONTEXT.md, and ADRs for current scope. Historical implementation plans under `docs/archive/ (historical plans removed from tree)` are not current visual direction. Do not use deleted files, old screenshots, generic dashboard examples, or the previous Raycast-inspired document as design authority.
