...
This repository has an agent-oriented product map under docs/openwiki/.
...
Read `docs/internal/design.md` (this directory’s `design.md`) before Masthead UI work. It is the single master design source for this repo.

Historical product scope lives in `docs/archive/prd.md` (read with ADR supersession notes). Prefer README, OpenWiki, CONTEXT.md, and ADRs for current boundaries. Historical workshop plans were removed from the public tree; see `docs/archive/` for retained provenance only. Do not resurrect the old Raycast-inspired design file or use archived screenshots as the current design contract unless Tyler explicitly asks for that.
...
For release-closeout work, use `../acceptance/product-release-gate.md` as the acceptance checklist.
