...
- `POST /masthead-pages/selection/resolve` accepts optional Logbook filters and currently returns the legacy authoritative `{ ok: true, artifactIds }` snapshot with at most 500 current eligible `session_dossier` IDs. The additive materialized resolver defines a discriminated complete/incomplete zero-partial-ID result, but Local 19 owns production cutover after shadow parity.
- `POST /masthead-pages/selection/materialized/resolve` is the dormant primary-only shadow surface for Local 19. It accepts the same optional filters and returns `{ ok: true, status: "complete", artifactIds }` or `{ ok: true, status: "incomplete", artifactIds: [], reason, retryable }`; incomplete responses never expose partial IDs. Secondary worktree bridges must not forward it.
...
- `POST /workbench/enroll-missing` adds non-deleted sessions that have no Workbench pipeline row. It never replays transcript import, quality classification, or enrichment from this request path; those remain owned by import and explicit Workbench operations. Sessions whose latest import health is partial or repair-required remain outside Workbench. Optional body: `{ "limit"?: number, "actorId"?: string }` (defaults limit `100` clamped 1–250, actor user `workbench_ui`). Returns `{ ok: true, enrolled, heldForImportRepair, skippedExisting, enrolledSessionIds, limit, generatedAt }`. The operation is idempotent, preserves existing published/manual/Not Added rows, and is primary-only / not bridge-safe.
