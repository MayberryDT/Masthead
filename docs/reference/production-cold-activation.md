...
The same public `abort --receipt <path>` operation cancels a valid candidate that never committed activation, including one stopped during offline preparation. Cancellation proves production offline, re-attests the receipt, candidate, finalized current bundle, unchanged active launch surfaces, and any nonce-bound database transition journal, then removes only that receipt's candidate, staged launchers, recovery stage, transition journal, and receipt. Database cancellation runs through the currently executing, content-attested lifecycle runtime rather than the staged candidate's maintenance binary, so a newer lifecycle can safely supersede receipts created before the cancellation protocol existed. A shared valid `backup-current`, the active database, and unrelated receipts or candidates remain untouched; if preparation had already migrated the active database, receipt-bound maintenance first restores its source snapshot under the database/runtime leases. Cancellation is journaled and idempotent under the lifecycle lease, so stale unactivated receipts can be cancelled one at a time before staging a replacement.
...
Offline prepare and restore each have a twelve-hour hard deadline, separate from the five-minute startup-health deadline. Preparation durably checkpoints `backup_copied`, `backup_verified`, `migration_stage_complete`, `post_migration_verified`, and `ready_to_activate`. A retry reuses the receipt-owned recovery stage or verified `backup-current`, checks its recorded filesystem identity before trusting prior work, and continues after the last checkpoint instead of repeating completed copies or hashes. A fresh preparation is bounded to one SQLite online-backup copy, one snapshot hash, source/target metadata attestation, migration/preflight work, and post-migration schema plus foreign-key validation. The normal prepare path does not run SQLite's unbounded page-structure integrity pragmas; the successful online backup and SHA-256 receipt bind the rollback bytes, while any path that actually restores those bytes still performs the full snapshot verification before promotion. On timeout Masthead sends `SIGTERM` only after revalidating the exact child PID/start identity; it never sends `SIGKILL`.
