...
Offline prepare and restore each have a twelve-hour hard deadline, separate from the five-minute startup-health deadline. Preparation durably checkpoints `backup_copied`, `backup_verified`, `migration_stage_complete`, `post_migration_verified`, and `ready_to_activate`. A retry reuses the receipt-owned recovery stage or verified `backup-current`, checks their recorded filesystem identity before trusting prior integrity work, and continues after the last checkpoint instead of repeating completed copies or scans. A fresh preparation is bounded to one snapshot copy, one full snapshot integrity pass, one snapshot hash, migration/preflight work, and one post-migration validation phase; a possible receipt-bound restore is a separate operation. On timeout Masthead sends `SIGTERM` only after revalidating the exact child PID/start identity; it never sends `SIGKILL`.
