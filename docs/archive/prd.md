...
> canonical dossier as an immutable snapshot. **ADR 0016** makes V5 authoring a durable guided request:
> the daemon fixes packs, returns evidence catalogs and blank skill fields, and publishes passing
> sessions without a canary or operator approval. V1–V4 remain readable audit history and cannot run
> or resume mutations. ADR 0011 and `CONTEXT.md` define the artifact-first
...
4. Follow daemon-grouped packs through evidence inspection, agent-authored enrichment, and
   flag-and-continue validation, then atomically publish rebuilt dossiers plus useful optional artifacts.
...
- **Enriched dossier:** The original canonical dossier structure remains the only dossier
  presentation. Under `workbench-authoring-v5`, the agent writes grounded durable enrichment for
  every pack session and the daemon rebuilds the dossier from canonical data.
- **Agent-led optional artifacts:** The agent may create zero or more runbooks, ADRs, and incident
  timelines from selected evidence. Knowledge opportunities are nonbinding; grounded yes/no
  consideration never requires a disposition or artifact.
- **Guided request:** Copy Agent Prompt creates a durable request before copying only its opaque ID
  and one instance-bound start command. The daemon, not the agent, fixes packs and returns one
  required next action at each state until the full-selection receipt exists.
- **No approval state:** V5 has no canary, operator approval, required opportunity disposition, or
  campaign-wide `needs_revision` state. Per-session rejects and warnings never stop the request.
- **Atomic publication:** Publication admits one accepted pack into Logbook atomically, and
