...
daemon_authoring_is_compatible() {
  ...
  EXPECTED_BASE_URL="http://127.0.0.1:$port" EXPECTED_CLI="$CLI_LAUNCHER" EXPECTED_MANIFEST="$INSTANCE_MANIFEST" "$NODE_BIN" -e 'let input = ""; const expected = ["bootstrap", "start", "claim", "inspect", "scaffold", "save", "finish", "status", "receipt"]; process.stdin.on("data", (chunk) => { input += chunk; }); process.stdin.on("end", () => { try { const j = JSON.parse(input); const valid = j?.capability === "artifact_authoring" && j?.protocol === "masthead.workbench.authoring/v1" && j?.bundleVersion === "workbench-authoring-v5" && j?.policyVersion === "workbench-authoring-v5" && j?.minimumSessionsPerPack === 5 && j?.maximumSessionsPerPack === 12 && j?.baseUrl === process.env.EXPECTED_BASE_URL && typeof j?.databaseId === "string" && j.databaseId === j.databaseId.trim() && j.databaseId.length > 0 && j?.command === process.env.EXPECTED_CLI && j?.instanceManifest === process.env.EXPECTED_MANIFEST && typeof j?.instanceId === "string" && j.instanceId === j.instanceId.trim() && j.instanceId.length > 0 && typeof j?.buildSha === "string" && j.buildSha === j.buildSha.trim() && j.buildSha.length > 0 && Array.isArray(j?.operations) && j.operations.length === expected.length && expected.every((operation, index) => j.operations[index] === operation); process.exit(valid ? 0 : 1); } catch { process.exit(1); } });' <<<"$capabilities"
}
